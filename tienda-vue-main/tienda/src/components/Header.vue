...
export default {
  ...
  created() {
    ...
    if (!this.sockets) {
      this.sockets = io('http://your-socket-server-url');
    }
    ...
    // Subscribe to the 'listen_cart' event
    this.sockets.on('listen_cart', (data) => {
      this.init_carrito();
  ...
  methods: {
    ...
    click_event() {
      this.sockets.emit('emit_method', 'Hola socket');
    }
  },
  beforeMount() {
    this.init_carrito();
